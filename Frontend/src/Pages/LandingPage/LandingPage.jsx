...
const LandingPage=()=>{
 ...
 <div className="feature-grid">{features.map((f,i)=><article className={`feature-card pro-card f-${i}`} key={f.title}><div className="feature-icon">{f.icon}</div><small>{f.tag}</small><h3>{f.title}</h3><p>{f.text}</p><Link to={f.to}>{f.cta}<FaArrowRight/></Link></article>)}<article className="feature-card video-card"><div className="video-heading"><span className="feature-icon"><FaVideo/></span><div><small>QUICK TOUR</small><h3>See the archive in action.</h3></div></div><iframe className="video" src="https://youtu.be/QVzch9eGvNk" title="20MIS archive walkthrough" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/></article></div></section>
